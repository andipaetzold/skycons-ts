...
export class Skycons {
    ...
    public constructor(opts: IConstructorOptions = {}) {
        ...
        this.color = {
            cloud: monochrome ? (color as string) : (color as IElementColors).cloud ?? "#666",
            fog: monochrome ? (color as string) : (color as IElementColors).fog ?? "#CCC",
            hail: monochrome ? (color as string) : (color as IElementColors).hail ?? "#CCF",
            leaf: monochrome ? (color as string) : (color as IElementColors).leaf ?? "#2C5228",
            light_cloud: monochrome ? (color as string) : (color as IElementColors).light_cloud ?? "#888",
            moon: monochrome ? (color as string) : (color as IElementColors).moon ?? "#353545",
            rain: monochrome ? (color as string) : (color as IElementColors).rain ?? "#7FDBFF",
            sleet: monochrome ? (color as string) : (color as IElementColors).sleet ?? "#C2EEFF",
            snow: monochrome ? (color as string) : (color as IElementColors).snow ?? "#C2EEFF",
            sun: monochrome ? (color as string) : (color as IElementColors).sun ?? "#FFDC00",
            thunder: monochrome ? (color as string) : (color as IElementColors).thunder ?? "#FFFF00",
            wind: monochrome ? (color as string) : (color as IElementColors).wind ?? "#777777",
        };
    }
